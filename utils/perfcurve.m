function [X,Y,T,auc,optrocpt,subY,subYnames] = ...
    perfcurve(cls,scores,posClass,NVPairs)
...
%         RNP   = (TN+ON)/(TP+FN+FP+TN) rate of negative predictions
%         accu  = (TP+TN)/(TP+FN+FP+TN) accuracy
%         TPR, sens, reca = TP/(TP+ON) true positive rate, sensitivity, recall
...
%               class J as class I. By default set to [0 1; 1 0].
%
...
%                 supported by BOOTCI except 'StdErr'. 'doc bootci' for more 
...
%                 for computing pointwise confidence bounds when you set NBOOT 
%                 to a positive integer. This argument can be created by a
%                 call to STATSET. PERFCURVE uses the following fields:
%                   'UseParallel'
%                   'UseSubstreams'
%                   'Streams'
...
arguments
    ...
    scores
    posClass
    NVPairs.NegClass = 'all'
    NVPairs.XCrit = 'fpr'
    NVPairs.YCrit = 'tpr'
    NVPairs.XVals = ''
    NVPairs.TVals = ''
    NVPairs.UseNearest matlab.lang.OnOffSwitchState = 'on'
    NVPairs.ProcessNaN = 'ignore'
    NVPairs.Prior = 'empirical'
    NVPairs.Cost = [0 1; 1 0];
    NVPairs.Weights = [];
    NVPairs.NBoot = 0;
    NVPairs.BootType = 'bca'
    NVPairs.BootArg = {};
    NVPairs.Alpha = .05
    NVPairs.Options = []
end

import mlearnlib.internal.utils.createOrThrowErrorWithStatsID
...
[negClass,xCrit,yCrit,xVals,tVals,toNearest,processNaN,prior,cost,...
    weights,nboot,boottype,bootarg,alpha,paropts] = dealNVArgs(NVPairs);

if ~ischar(xCrit) && ~isa(xCrit,'function_handle')
    createOrThrowErrorWithStatsID('stats:perfcurve:BadXCrit');
end

if ~ischar(yCrit) && ~isa(yCrit,'function_handle')
    createOrThrowErrorWithStatsID('stats:perfcurve:BadYCrit');
end

if ~isempty(xVals) && ...
        ~(ischar(xVals) && strcmpi(xVals,'all')) && ~isnumeric(xVals)
    createOrThrowErrorWithStatsID('stats:perfcurve:BadXVals');
end

if ~isempty(tVals) && ...
        ~(ischar(tVals) && strcmpi(tVals,'all')) && ~isnumeric(tVals)
    createOrThrowErrorWithStatsID('stats:perfcurve:BadTVals');
end

toNearest = logical(toNearest);

if ~ischar(processNaN)
    createOrThrowErrorWithStatsID('stats:perfcurve:BadProcessNaNType');
end
allowedVals = {'ignore' 'addtofalse'};
tf = strncmpi(processNaN,allowedVals,length(processNaN));
if sum(tf)~=1
    createOrThrowErrorWithStatsID('stats:perfcurve:BadProcessNaNValue');
end

if ~ischar(prior) && (~isfloat(prior) || ~isvector(prior) || numel(prior)~=2)
    createOrThrowErrorWithStatsID('stats:perfcurve:BadPrior');
end
if ischar(prior)
    allowedVals = {'empirical' 'uniform'};
    tf = strncmpi(prior,allowedVals,length(processNaN));
    if sum(tf)~=1
        createOrThrowErrorWithStatsID('stats:perfcurve:BadPriorString');
    end
end

if ~isfloat(cost) || ~ismatrix(cost) || ~isequal(size(cost),[2 2])
    createOrThrowErrorWithStatsID('stats:perfcurve:BadCost');
end

if ~isempty(weights) && ~isfloat(weights) && ~iscell(weights)
    createOrThrowErrorWithStatsID('stats:perfcurve:BadWeights');
end

if ~isnumeric(nboot) || ~isscalar(nboot) || nboot<0
    createOrThrowErrorWithStatsID('stats:perfcurve:BadNBoot');
end

if ~ischar(boottype)
    createOrThrowErrorWithStatsID('stats:perfcurve:BadBootType');
end

if ~iscell(bootarg)
    createOrThrowErrorWithStatsID('stats:perfcurve:BadBootArg');
end

if ~isfloat(alpha) || ~isscalar(alpha) || alpha<=0 || alpha>=1
    createOrThrowErrorWithStatsID('stats:perfcurve:BadAlpha');
end
...
if ~isempty(tVals) && ~isempty(xVals)
    createOrThrowErrorWithStatsID('stats:perfcurve:BothTandXsupplied');
end
...
% If X values or thresholds are given as empty numeric matrices, treat as
% default values
if isnumeric(xVals) && isempty(xVals)
    xVals = '';
end

if isnumeric(tVals) && isempty(tVals)
    tVals = '';
end
...
% SMLT is required to compute CI - verify there is a SMLT license present
% and the toolbox is installed
useParallel = false;
if (docv || doboot)
    if ~builtin('license', 'test', 'Statistics_Toolbox') || ~builtin('license', 'checkout', 'Statistics_Toolbox') || isempty(which('statset'))
         createOrThrowErrorWithStatsID('stats:perfcurve:StatsRequiredForCI');
    else
        if isempty(paropts)
            paropts = statset('rocmetrics');
        end
...
if docv && doboot
    createOrThrowErrorWithStatsID('stats:perfcurve:BothCVandBootstrapRequested');
end
nsub = max(ncv,nboot);
...
if numel(trueNames) < 2
    createOrThrowErrorWithStatsID('stats:perfcurve:NotEnoughClasses');
end

% Check costs
if (cost(2,1)-cost(2,2))<=0 || (cost(1,2)-cost(1,1))<=0
    createOrThrowErrorWithStatsID('stats:perfcurve:InvalidCost');
end
...
if (isa(xCrit,'function_handle') || isa(yCrit,'function_handle')) && docv
    createOrThrowErrorWithStatsID('stats:perfcurve:UserCritConfBounds');
end
...
numObs = sum(Wcum(end,:));
[fx, fwx, xcritBounds] = makeCrit(xCrit, numObs);
[fy, fwy, ycritBounds] = makeCrit(yCrit, numObs);
...
if any(isnan(X(~special)))
    createOrThrowErrorWithStatsID('stats:perfcurve:BadXCritValue');
end
if any(isnan(Y(~special)))
    createOrThrowErrorWithStatsID('stats:perfcurve:BadYCritValue');
end
...
if docv || doboot
    ...
    if     docv % cross-validation
        ...
        if useTVals
            ...
            ciX(ciX < xcritBounds(1)) = xcritBounds(1);
            ciX(ciX > xcritBounds(2)) = xcritBounds(2);
        end
        ...
        ciY(ciY < ycritBounds(1)) = ycritBounds(1);
        ciY(ciY > ycritBounds(2)) = ycritBounds(2);
    elseif doboot
...
if nargout>3
    ...
    if docv || doboot
        if     docv
            ...
            % Specific AUC bounds depend on the combination of criteria
            % We can cap at a guaranteed max and min value based on the
            % two criteria chosen, even if there isn't actually any way to
            % exceed the computed min/max
            xLength = (xcritBounds(2) - xcritBounds(1));
            maxArea = ycritBounds(2) * xLength; 
            minArea = ycritBounds(1) * xLength;
            ciauc(ciauc < minArea) = minArea;
            ciauc(ciauc > maxArea) = maxArea;
        elseif doboot
            bootfun = @(idx) ...
...
posClass = mlearnlib.internal.utils.ClassLabel(posClass);
if numel(posClass)>1
    createOrThrowErrorWithStatsID('stats:perfcurve:TooManyPositiveClasses');
end
if ~ismember(posClass,trueNames)
    createOrThrowErrorWithStatsID('stats:perfcurve:PositiveClassNotFound');
end
...
if strcmpi(negClass,'all')
    ...
    negClass = mlearnlib.internal.utils.ClassLabel(negClass);
    tf = ismember(negClass,trueNames);
    if any(~tf)
        createOrThrowErrorWithStatsID('stats:perfcurve:NegativeClassNotFound');
    end
    tf = ismember(posClass,negClass);
    if tf
        createOrThrowErrorWithStatsID('stats:perfcurve:PositiveAndNegativeClassesOverlap');
    end
end
...
if nNeg~=numel(levels(negClass))
    createOrThrowErrorWithStatsID('stats:perfcurve:DuplicateNegativeClasses');
end
...
scale = zeros(2,1,'like',Wcum);
Wpos = Wcum(end,1);
Wneg = sum(Wcum(end,2:end),2);
if ischar(prior) && strcmpi(prior,'empirical')
    scale = ones(2,1,'like',Wcum);
end
if ischar(prior) && strcmpi(prior,'uniform')
    prior = ones(2,1,'like',Wcum);
end
if isnumeric(prior)
    if any(prior<=0)
        mlearnlib.internal.utils.createOrThrowErrorWithStatsID('stats:perfcurve:NonPositivePriors');
    end
...
function [f,wf, bounds] = makeCrit(crit, numObs)
...
if isa(crit,'function_handle')
    ...
    bounds = [-Inf, Inf]; % Arbitrary metric - can't place bounds on it
    return;
end
...
switch lower(crit)
    ...
    case {'ecost', 'expectedcost'}
        ...
        bounds = [-Inf, Inf];
    otherwise
        mlearnlib.internal.utils.createOrThrowErrorWithStatsID('stats:perfcurve:UnknownXYCriterion');
end
end
...
if ~isempty(XorTrange)
    if numel(XorTrange)~=2
        mlearnlib.internal.utils.createOrThrowErrorWithStatsID('stats:perfcurve:InvalidXorTRange');
    end
    ...
    if isempty(inrange)
        mlearnlib.internal.utils.createOrThrowErrorWithStatsID('stats:perfcurve:XorTRangeTooRestrictive');
    end
end
end
...
% Init
T = zeros(length(div),1,'like',scores);
...
if any(vals(1:end-1)>vals(2:end))
    increasing = -1;
    if any(vals(1:end-1)<vals(2:end))
        mlearnlib.internal.utils.createOrThrowErrorWithStatsID('stats:perfcurve:NonMonotoneXCriterion');
    end
end
end
...
if any(div>Nrow)
    mlearnlib.internal.utils.createOrThrowErrorWithStatsID('stats:perfcurve:BadDivisionIndex');
end
...
wP = Wcum(end,1);
subY = zeros(length(tpX),nNegClass,'like',Wcum);
for i=1:nNegClass
    wN = Wcum(end,i+1);
    if wN==0
        mlearnlib.internal.utils.createOrThrowErrorWithStatsID('stats:perfcurve:BadClassCounts', i);
    end
    fpX = Wcum(divX,i+1);
...
if iscell(scores) % Cross-validated scores and labels
    % Scores
    if ~isvector(scores)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVScoresNotVector');
    end
    scores = scores(:);
    ncv = numel(scores);
    if ncv<2
        createOrThrowErrorWithStatsID('stats:perfcurve:CVScoresTooShort');
    end
    ...
    if any(tfisemp) || any(~tfisnum) || any(~tfisvec)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVScoresWithBadElements');
    end
    nels = cellfun(@numel,scores);
    for icv=1:ncv
        scores{icv} = scores{icv}(:);
    end
    supFloat = superiorfloat(scores{:});
    
    % Labels
    if ~iscell(cls)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVLabelsNotCell');
    end
    if ~isvector(cls)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVLabelsNotVector');
    end
    if numel(cls)~=ncv
        createOrThrowErrorWithStatsID('stats:perfcurve:CVLabelsWithNonmatchingLength');
    end
    cls = cls(:);
    ltype = cellfun(@class,cls,'UniformOutput',false);
    for icv=2:ncv
        if ~strcmp(ltype{1},ltype{icv})
            createOrThrowErrorWithStatsID('stats:perfcurve:CVLabelsWithDifferentTypes');
        end
    end
    nell = zeros(ncv,1);
    for icv=1:ncv
        ...
        cls{icv} = mlearnlib.internal.utils.ClassLabel(cls{icv});
    ...
    if isempty(weights)
        ...
        if ~iscell(weights)
            createOrThrowErrorWithStatsID('stats:perfcurve:CVWeightsNotCell');
        end
        if ~isvector(weights)
            createOrThrowErrorWithStatsID('stats:perfcurve:CVWeightsNotVector');
        end
        if numel(weights)~=ncv
            createOrThrowErrorWithStatsID('stats:perfcurve:CVWeightsWithNonmatchingLength');
        end
        weights = weights(:);
    end
    for icv=1:ncv
        if isempty(weights{icv})
            weights{icv} = ones(nels(icv),1,supFloat);
        else
    ...
    if any(~tfisflo) || any(~tfisvec)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVWeightsNotNumeric');
    end
    fneg = @(x) any(x<0);
    fzer = @(x) all(x==0);
    fnan = @(x) all(isnan(x));
    ...
    if any(tfisneg) || any(tfiszer) || any(tfisnan)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVWeightsNegativeOrNaN');
    end
    nelw = cellfun(@numel,weights);
    
    % Check sizes
    if any(nels~=nell)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVLabelsNotMatchedToScores');
    end

    if any(nelw~=nell)
        createOrThrowErrorWithStatsID('stats:perfcurve:CVWeightsNotMatchedToScores');
    end
    ...
    % Standardize type
    supFloat = superiorfloat(cast(1, supFloat), weights{:});
    scores = cellfun(@(s) cast(s, supFloat), scores, 'UniformOutput', false);
    weights = cellfun(@(w) cast(w, supFloat), weights, 'UniformOutput', false);
else % Plain scores, labels and weights
    % Scores
    if ~isfloat(scores) || ~isvector(scores)
        createOrThrowErrorWithStatsID('stats:perfcurve:BadScores');
    end
    scores = scores(:);
    ns = numel(scores);
    ...
    cls = mlearnlib.internal.utils.ClassLabel(cls);
    nl = numel(cls);
    if ns~=nl
        createOrThrowErrorWithStatsID('stats:perfcurve:ScoresAndLabelsDoNotMatch');
    end
    
    % Weights
    if isempty(weights)
        weights = ones(ns,1,'like',scores);
    else
        if ~isfloat(weights)
            createOrThrowErrorWithStatsID('stats:perfcurve:WeightsNotRealNumbers');
        end
        if ~isvector(weights)
            createOrThrowErrorWithStatsID('stats:perfcurve:WeightsNotVector');
        end
        if any(weights<0) || all(weights==0)
           createOrThrowErrorWithStatsID('stats:perfcurve:WeightsNegative');
        end
        if numel(weights)~=ns
            createOrThrowErrorWithStatsID('stats:perfcurve:WeightsNotMatchedToScores');
        end
        if any(isnan(weights))
            createOrThrowErrorWithStatsID('stats:perfcurve:NaNWeights');
        end
        weights = weights(:);
        iszer = weights==0;
        scores(iszer) = [];
        ...
        supFloat = superiorfloat(scores, weights);
        cast(scores, supFloat);
        cast(weights, supFloat);
    end
end
end
...
if ~all(sum(Wcum,1)) && notifyClassAbsent()
    warning('stats:perfcurve:SubSampleWithMissingClasses', getString(message('mlearnlib:perfcurve:SubSampleWithMissingClasses')));
end
...
tfzeroW = ~any(W,2);
if any(tfzeroW)
    warning('stats:perfcurve:CVfoldsWithZeroWeights', getString(message('mlearnlib:perfcurve:CVfoldsWithZeroWeights')));
end
E = zeros(length(M),1,'like',W);
i = ~tfzeroW;
E(i) = sqrt(sum(W(i,:).*(X(i,:) - M(i)).^2,2) ./ sum(W(i,:),2));
end
...
function [negClass,xCrit,yCrit,xVals,tVals,toNearest,processNaN,prior,cost,...
    weights,nboot,boottype,bootarg,alpha,paropts] = dealNVArgs(NVPairs)
% This function converts from a struct of NV pairs to individual variables.
% It also converts strings to chars as relevant.
negClass = convertStringsToChars(NVPairs.NegClass);
xCrit = convertStringsToChars(NVPairs.XCrit);
yCrit = convertStringsToChars(NVPairs.YCrit);
xVals = convertStringsToChars(NVPairs.XVals);
tVals = convertStringsToChars(NVPairs.TVals);
toNearest = convertStringsToChars(NVPairs.UseNearest);
processNaN = convertStringsToChars(NVPairs.ProcessNaN);
prior = convertStringsToChars(NVPairs.Prior);
cost = NVPairs.Cost;
weights = NVPairs.Weights;
nboot = NVPairs.NBoot;
boottype = convertStringsToChars(NVPairs.BootType);
bootarg = NVPairs.BootArg;
alpha = NVPairs.Alpha;
paropts = NVPairs.Options;
end
